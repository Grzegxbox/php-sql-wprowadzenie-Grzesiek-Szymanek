...
<?php
  echo("<h3>1. Średnia Zarobków W Poszczególnych Działach Mniejsza Od 28</h3>");
      ...
      $result = $conn -> query('SELECT dzial, AVG(zarobki) AS srednia, nazwa_dzialu FROM pracownicy, organizacja WHERE (dzial=id_org) GROUP BY dzial HAVING AVG(zarobki)<28');
          echo("<table border=1>");
          echo("<th>DZIAŁ</th>");
          echo("<th>ŚREDNIA</th>");
  ...
  echo("<h3>3. Ilość Pracowników W Poszczególnych Działach Większa Od 3</h3>");
      $conn = new mysqli("remotemysql.com","Rp4CxP6YkY","V0BMRFi2V3","Rp4CxP6YkY");
      $result = $conn -> query('SELECT dzial, COUNT(imie) AS ilosc, nazwa_dzialu FROM pracownicy, organizacja WHERE (dzial=id_org) GROUP BY dzial HAVING COUNT(imie)<3');
          echo("<table border=1>");
          echo("<th>DZIAŁ</th>");
          echo("<th>ILOŚĆ</th>");
          echo("<td>NAZWA DZIAŁU</td>");
              while($row = $result -> fetch_assoc()){
                  echo("<tr>");
                      echo("<td>" .$row["dzial"]. "</td><td>" .$row["ilosc"]. "</td><td>" .$row["nazwa_dzialu"]. "</td>");
                  echo("</tr>");
              }
          echo("</table>");
?>
